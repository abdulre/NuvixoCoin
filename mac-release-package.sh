...
PACKAGE=NXC-client-${VERSION}
echo PACKAGE="${PACKAGE}"
CHANGELOG=NXC-client-${VERSION}.changelog.txt
OBFUSCATE=$2
MACVERSION=$3
if [ -x ${MACVERSION} ];
then
...
FILES="${FILES} NuvixoCoin.exe NuvixoCoinservice.exe"
...
echo copy resources
cp installer/lib/JavaExe.exe NuvixoCoin.exe
cp installer/lib/JavaExe.exe NuvixoCoinservice.exe
cp -a ${FILES} nxt
