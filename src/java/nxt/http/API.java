...
public final class API {
    ...
    // ── NuvixoCoin: enforce non-empty admin password on public interfaces ─────
    static {
        String host = Nxt.getStringProperty("nxt.apiServerHost", "127.0.0.1");
        boolean isLocalhost = "127.0.0.1".equals(host) || "localhost".equals(host);
        if (!isLocalhost && (adminPassword == null || adminPassword.trim().isEmpty()
                || "CHANGE_THIS_BEFORE_LAUNCH".equals(adminPassword.trim()))) {
            throw new RuntimeException(
                "[NuvixoCoin] nxt.adminPassword must be set to a strong password before " +
                "exposing the API server publicly. Set nxt.apiServerHost=127.0.0.1 for local-only mode.");
        }
    }
    ...
    static {
        ...
        if (enableAPIServer) {
            ...
            // NuvixoCoin: block startup if admin password is default placeholder or empty on public interface
            boolean isLocalHost = "127.0.0.1".equals(host) || "localhost".equals(host);
            if (!isLocalHost && (adminPassword.isEmpty() || "CHANGE_THIS_BEFORE_LAUNCH".equals(adminPassword))) {
                throw new RuntimeException(
                    "[NuvixoCoin] Set nxt.adminPassword to a strong password before exposing the API server. " +
            ...
            //
            // Jetty 11: SslContextFactory bare constructor removed — must use .Server or .Client
            ...
            if (apiServerCORS) {
                ...
                // NuvixoCoin: restrict CORS to configured origins instead of wildcard
                String allowedOrigins = Nxt.getStringProperty("nxt.apiAllowedOrigins",
                        "http://localhost:" + port + ";http://127.0.0.1:" + port);
                filterHolder.setInitParameter(CrossOriginFilter.ALLOWED_ORIGINS_PARAM, allowedOrigins.replace(";", ","));
                filterHolder.setInitParameter(CrossOriginFilter.ALLOWED_METHODS_PARAM, "GET,POST,OPTIONS");
                filterHolder.setInitParameter(CrossOriginFilter.ALLOWED_HEADERS_PARAM, "Content-Type,Authorization");
