        ...
        try (Statement stmt = conn.createStatement()) {
            ...
            try (ResultSet rs = stmt.executeQuery("SHOW COLUMNS FROM " + table + " FROM " + schema)) {
                ...
                while (rs.next()) {
                    ...
                    // H2 2.x reports a bare type name (no parens) for columns with no
                    // explicit precision/length, e.g. a plain "VARCHAR" column shows up
                    // as "VARCHAR" rather than "VARCHAR(N)" the way H2 1.x always did.
                    // Only strip the parenthesized suffix when one is actually present.
            ...
            try (ResultSet rs = stmt.executeQuery(String.format(
                ...
                if (rs.next()) {
                    ...
                    for (String column : columns) {
                        ...
                        if (pos >= 0) {
                            // H2 2.x reports the ANSI-standard type name "CHARACTER VARYING"
                            // from SHOW COLUMNS instead of "VARCHAR", which H2 1.x always used
                            // (confirmed directly against H2's own DataType.java, which
                            // registers "CHARACTER VARYING" as the primary name for
                            // Value.VARCHAR). Accepting both keeps this working regardless
                            // of which H2 version/format is actually in play.
                            String columnTypeName = columnTypes.get(pos);
